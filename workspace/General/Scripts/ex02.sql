...
WHERE SALARY >= 10000   -- 2. SALARY을 찾아서 10000 이상만
...
SELECT SALARY 
FROM EMPLOYEES e
WHERE SALARY = ANY(10000, 11000, 12000);  --IN과 같다.
...
--INSERT INTO TBL_MEMBER (순서 지정해줘야함)
--VALUES(11, '아무무'); 안댐요  
...
5, '신짱구', 22, 13, 9
*/
INSERT INTO TBL_STUDENT (STUDENT_ID,NAME,MATE,ENG,KOR) 
...
UPDATE TBL_STUDENT 
SET GRADE = 'A'
WHERE (MATE + ENG + KOR )/3 >=90;

UPDATE TBL_STUDENT 
SET GRADE = 'B'
WHERE (MATE + ENG + KOR )/3 >= 80 AND (MATE + ENG + KOR )/3 <90;

UPDATE TBL_STUDENT 
SET GRADE = 'C'
WHERE (MATE + ENG + KOR )/3 >=50 AND (MATE + ENG + KOR )/3 < 80;

UPDATE TBL_STUDENT 
SET GRADE = 'F'
WHERE (MATE + ENG + KOR )/3 <50;
...
*/
SELECT STUDENT_ID 번호,
	NAME 이름,
	ROUND((MATE + ENG + KOR )/3,2) 평균
FROM TBL_STUDENT ts ;
