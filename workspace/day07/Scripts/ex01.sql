...
-----------------------------------------------
-- [실습]
-- PLAYER 테이블에서 송중국 선수가 속한 팀의 전화번호 조회
...
SELECT J.JOB_TITLE ,
	E.EMAIL ,E.LAST_NAME ,E.FIRST_NAME 
...
SELECT *
FROM (SELECT TEAM_ID , MAX(HEIGHT) 
	FROM PLAYER p 
	GROUP BY TEAM_ID) S
	JOIN PLAYER p 
ON S.TEAM_ID = P.TEAM_ID AND P.HEIGHT = ;
