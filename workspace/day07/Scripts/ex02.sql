...
SELECT *
FROM TBL_USER U RIGHT OUTER JOIN TBL_GRADE G
ON U.GRADE_ID = G.GRADE_ID ;

-- LEFT OUTER JOIN 
-- 우선 왼쪽 테이블의 정보를 전부 가져온 후에 ON조건과 일치하는 행을 병합
-- RIGHT OUTER JOIN
-- 우선 오른쪽 테이블이 정보를 전부 가져온 후에 ON조건과 일치하는 행을 병합

-- 카테시안 곱, CROSS JOIN 
-- 효율이 최악
SELECT * 
FROM TBL_USER U , TBL_GRADE G
WHERE USER_ID = 1 AND G.GRADE_ID = 1; 

------------------------------------------
/*
 * PLAYER 테이블에서 키가 NULL인 선수들의 키를 170으로 
 * 변경하여 평균 조회 (UPDATE 아님! SELECT로만 하기)
 */
...
/*
 * AVG함수를 쓰지 않고 PLYAER 테이블에서 선수들의 평균 키 구하기
 */
...
/*
 * DEPT 테이블의 LOC별 평균 급여를 반올림한 결과 각 LOC별 SAL합을 조회
 */
...
ON P.WEIGHT = S.WEIGHT AND P.TEAM_ID = S.TEAM_ID
ORDER BY P.TEAM_ID ;
